...
/*   Updated: 2022/01/03 10:09:56 by oairola          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	reader(int fd, char **line, char **strsaver)
{
	...
	if (!ft_check_newl(str, 1))
	{
		while (1)
		{
			...
			if (ret < BUFF_SIZE)
			{
				temp = str;
				free(str);
				str = ft_strjoin_eof(temp, buf, ret);
				free(temp);
			}
			else
			{
	...
	*line = ft_strsub(str, 0, ft_check_newl(str, 2));
	if (ft_check_newl(str, 1))
	{
		strsaver[fd] = ft_strncpy(strsaver[fd], str, ft_check_newl(str, 2)); /* ONGELMA: strsaver[fd] ja str samassa osoitteessa, kun poistaa str:n myös strsaver poistuu*/
		strsaver[fd]++;
	}
	else
		ft_strclr(strsaver[fd]);
